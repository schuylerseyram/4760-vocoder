...
typedef signed int fix19;
#define multfix19(a, b) ((fix19)((((signed long long)(a)) * ((signed long long)(b))) >> 19))
#define float2fix19(a) ((fix19)((a) * 524288.0))
#define fix2float19(a) ((float)(a) / 524288.0)
#define absfix19(a) abs(a)
#define int2fix19(a) ((fix19)(a << 19))
#define fix2int19(a) ((int)(a >> 19))
#define char2fix19(a) (fix19)(((fix19)(a)) << 19)
#define divfix(a, b) (fix19)((((signed long long)(a)) << 19) / (b))
...
int current_carrier;
fix19 sawtooth_carrier_table[carrier_table_size];
fix19 whitenoise_carrier_table[carrier_table_size];
fix19 triangle_carrier_table[carrier_table_size];
...
fix19 max_amplitude = int2fix19(1); // maximum amplitude
fix19 attack_inc;                   // rate at which sound ramps up
fix19 decay_inc;                    // rate at which sound ramps down
fix19 current_amplitude_0 = 0;      // current amplitude (modified in ISR)
fix19 current_amplitude_1 = 0;      // current amplitude (modified in ISR)
fix19 sine_scale = 0;
...
fix19 lp_ysamp[NUM_FILTERS];
fix19 lp_ysig[NUM_FILTERS];

fix19 coefficients[NUM_FILTERS][2][5];

fix19 yy;
fix19 xn;
fix19 xn_1arrSamp[NUM_FILTERS];
fix19 xn_2arrSamp[NUM_FILTERS];
fix19 xn_1arrSig[NUM_FILTERS];
fix19 xn_2arrSig[NUM_FILTERS];
fix19 yn_1arrSamp[NUM_FILTERS];
fix19 yn_2arrSamp[NUM_FILTERS];
fix19 yn_1arrSig[NUM_FILTERS];
fix19 yn_2arrSig[NUM_FILTERS];
...
fix19 Butter2band(fix19 xx, fix19 b1, fix19 a2, fix19 a3, fix19 *xn_2, fix19 *xn_1, fix19 *yn_2, fix19 *yn_1)
{
    yy = multfix19((xx - *xn_2), b1);
    *xn_2 = *xn_1;
    *xn_1 = xx;
    yy = yy - multfix19(*yn_2, a3);
    *yn_2 = *yn_1;
    yy = yy - multfix19(*yn_1, a2);
    *yn_1 = yy;
    return yy;
}

fix19 lowpass(fix19 x, fix19 *lp_y1)
{
    fix19 lp_yy = x + ((*lp_y1 - x) - ((*lp_y1 - x) >> 8));
    // fix19 lp_yy = x + ((*lp_y1 - x)>>6);
    *lp_y1 = lp_yy;
    return lp_yy;
}
...
bool repeating_timer_callback_core_0(struct repeating_timer *t)
{
    ...
    adc_data = int2fix19((int)result);
    new_adc = 1;

    // DAC_output_0 = fix2int19(lowpass(absfix19(adc_data), &(lp_ysamp[0])));
...
static PT_THREAD(protothread_vocoder(struct pt *pt))
{
    ...
    while (1)
    {
        ...
        // fix19 carrier_sample = carrier_table[phase_accum_main_0];
        ...
        for (i = 0; i < NUM_FILTERS; i++)
        {
            ...
            fix19 sample = Butter2band(adcd, coefficients[i][0][0], coefficients[i][1][1], coefficients[i][1][2],
                                       &(xn_2arrSamp[i]), &(xn_1arrSamp[i]), &(yn_2arrSamp[i]), &(yn_1arrSamp[i]));
            fix19 amplifiedSample = lowpass(absfix19(sample), &(lp_ysamp[i]));
            fix19 filteredSig = Butter2band(carrier_sample, coefficients[i][0][0], coefficients[i][1][1], coefficients[i][1][2],
                                            &(xn_2arrSig[i]), &(xn_1arrSig[i]), &(yn_2arrSig[i]), &(yn_1arrSig[i]));
            fix19 amplifiedSig = lowpass(absfix19(filteredSig), &(lp_ysig[i]));
            y += amplifiedSig != 0 ? multfix19(filteredSig, divfix(amplifiedSample, amplifiedSig)) : 0;
...
int main()
{
    ...
    for (i = 0; i < NUM_FILTERS; i++)
    {
        for (j = 0; j < 2; j++)
        {
            for (k = 0; k < 3; k++)
            {
                coefficients[i][j][k] = float2fix19(float_coefficients[i][j][k]);
            }
        }
    }
    ...
    attack_inc = divfix(max_amplitude, int2fix19(ATTACK_TIME));
    decay_inc = divfix(max_amplitude, int2fix19(DECAY_TIME));
    ...
    {
        // sawtooth
        sawtooth_carrier_table[ii] = float2fix19(((float)ii) / (carrier_table_size - 1));
        // sine wave
        // carrier_table[ii] = float2fix19(0.5 + 0.5 * sin((2 * M_PI * ii) / carrier_table_size));
        // pulse
        // carrier_table[ii] = float2fix19(ii < 4096 ? 1.0 : 0.0);
        // square
        if (ii < 4096)
        {
            triangle_carrier_table[ii] = float2fix19(ii * (1.0 / 4096.0));
        }
        else
        {
            triangle_carrier_table[ii] = float2fix19(1.0 - (ii - 4096) * (1.0 / 4096));
        }

        // white noise
        whitenoise_carrier_table[ii] = float2fix19(((rand() % 100) / 100.0));
    }
